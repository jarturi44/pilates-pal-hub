  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [user, intake, waiver, sub, plans, completions, attendance, allSlots, mySlots, fulfill, notifs, warmups] = await Promise.all([
        ...
        supabase.from("subscriptions").select("*, plan:plans(*)").eq("user_id", clientId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
        supabase.from("plans").select("*").order("price_per_month"),
